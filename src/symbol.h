#ifndef GPU_CUDA_SYMBOL_H
#define GPU_CUDA_SYMBOL_H
...
void _copy_to_symbol(const std::string &name, const T &value)
{
    ...
    if(sizeof(T) > size_storage)
        nvrThrow(std::runtime_error("'"+name+"'"+" storage overflow"));

    cudaMemcpyToSymbol(name.c_str(), &value, sizeof(T));
...
#if CUDA_VERSION >= 5050
    cudaMemcpyToSymbol(*symbol,&items[0], size);
#else
    cudaMemcpyToSymbol(symbol,&items[0], size);
#endif
    check_cuda_error("Error copying symbol to device");
...
#if CUDA_VERSION >= 5050
    cudaMemcpyToSymbol(*symbol,&*beg, size);
#else
    cudaMemcpyToSymbol(symbol,&*beg, size);
#endif
    check_cuda_error("Error copying symbol to device");
                    ...
                    const std::vector<T> &items)
{
    ...
    cudaMemcpyToSymbol(name.c_str(),&items[0], size);
    check_cuda_error("Error copying '"+name+"' buffer to device");
